...
import { fetchParentProfile, saveParentProfile, type ParentProfileData } from '@/lib/parentProfileActions';
import { saveNotifPrefs } from '@/lib/studentProfileActions';
...
export function ParentProfile() {
  ...
  const [d, setD] = useState<ParentProfileData | null>(null);

  const load = useCallback(async () => {
    if (!uid) return;
    setD(await fetchParentProfile(uid)); setLoading(false);
  }, [uid]);
  useFocusEffect(useCallback(() => { load(); }, [load]));

  const set = (patch: Partial<ParentProfileData>) => setD((cur) => (cur ? { ...cur, ...patch } : cur));
  const flash = (m: string) => { setToast(m); setTimeout(() => setToast(null), 2500); };

  const completion = useMemo(() => {
    if (!d) return 0;
    const items = [d.firstName, d.lastName, d.phone, d.country, d.familyBio].filter((v) => v && String(v).trim());
    return Math.round((items.length / 5) * 100);
  }, [d]);

  if (loading || !d) return <Screen scroll={false}><Loading label="Loading profile…" /></Screen>;

  const initials = `${d.firstName.charAt(0)}${d.lastName.charAt(0)}`.toUpperCase() || 'P';
  ...
  async function save() {
    ...
    setSaving(true);
    const ok = await saveParentProfile(uid, d!);
  ...
  return (
    <Screen>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.idBody}>
          ...
          <View style={styles.nameRow}>
            ...
            <View style={styles.rolePill}><Text style={styles.rolePillText}>Parent</Text></View>
          </View>
      ...
      {/* 3 — Stats */}
      <StatCard icon="people-outline" label="CHILDREN" value={d.stats.children} />
      <StatCard icon="checkmark-circle-outline" label="LESSONS" value={d.stats.lessons} />
      <StatCard icon="calendar-outline" label="BOOKINGS" value={d.stats.bookings} />

      {/* 5 — Badges */}
      <Section icon="trophy-outline" title="Your Badges" subtitle="Earned through your children’s learning journey">
        <Text style={styles.muted}>Earn badges as your children attend lessons and progress in their learning.</Text>
      </Section>

      {/* 6 — Personal info */}
      <Section icon="person-outline" title="Personal Information" subtitle="Your name as it appears to teachers and staff">
        ...
        <Field label="About Your Family" hint="Tell us about your family and your children’s learning goals.">
          <Input value={d.familyBio} onChangeText={(v) => set({ familyBio: v })} placeholder="I have two children, ages 7 and 10, starting with Noorani Qaida and Hifz…" multiline />
        </Field>
      </Section>

      {/* 7 — Contact & location */}
      <Section icon="globe-outline" title="Contact & Location" subtitle="How we reach you and schedule your children’s lessons">
      ...
      <Section icon="lock-closed-outline" title="Account" subtitle="Login & membership">
        ...
        <Row label="Account type" value="Parent" />
        <Row label="Member since" value={d.memberSince || '—'} last />
      </Section>
      ...
      <LinearGradient colors={['#16291E', '#166534']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.quote}>
        <Text style={styles.quoteAr}>رَبِّ اجْعَلْنِي مُقِيمَ الصَّلَاةِ وَمِن ذُرِّيَّتِي</Text>
        <Text style={styles.quoteEn}>"My Lord, make me an establisher of prayer, and [many] from my descendants"</Text>
        <Text style={styles.quoteRef}>— Surah Ibrahim 14:40</Text>
      </LinearGradient>
