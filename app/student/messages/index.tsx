// app/student/messages/index.tsx — conversations list.
...
export default function Messages() {
  ...
  return (
    <Screen>
      ...
      {convos.length === 0 ? (
        <EmptyCard icon="chatbubbles-outline" title="No messages yet" body="When you book a teacher, your conversations will appear here." />
      ) : (
        convos.map((c) => (
          <Pressable key={c.id} onPress={() => router.push(`/student/messages/${c.id}` as any)} style={({ pressed }) => [styles.row, pressed && { opacity: 0.85 }]}>
