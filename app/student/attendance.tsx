// app/student/attendance.tsx — student Attendance (mirrors web AttendanceDashboard):
// insight (streak + trend), attendance ring, P/L/A/E stats, monthly bar chart,
// colour-coded Month/Week calendar, and history. Read-only (teacher marks it).
...
export default function StudentAttendance() {
  ...
  const load = useCallback(async () => {
    ...
    setRecords(await fetchStudentAttendance(session.user.id)); setLoading(false);
  }, [session?.user?.id]);
  ...
  if (records.length === 0) {
    return (
      <Screen>
        <Header />
        <View style={styles.empty}><Ionicons name="calendar-outline" size={30} color={C.gold} /><Text style={styles.emptyText}>Your attendance will appear here once your teacher marks your lessons.</Text></View>
      </Screen>
    );
  }
    ...
    <Screen>
      ...
      <View style={styles.card}>
        <View style={styles.ringWrap}>
          <View style={styles.ring}><Text style={styles.ringPct}>{ins.rate}%</Text><Text style={styles.ringLabel}>attendance</Text></View>
        </View>
...
function Header() {
  return (
    <>
      <Text style={styles.eyebrow}>MY LEARNING</Text>
      <Text style={styles.h1}>Attendance</Text>
      <Text style={styles.sub}>Track your learning consistency across all your lessons.</Text>
    </>
  );
}
...
const styles = StyleSheet.create({
  ...
  ring: { width: 130, height: 130, borderRadius: 65, borderWidth: 10, borderColor: C.gold, alignItems: 'center', justifyContent: 'center' },
  ringPct: { fontFamily: FONT.displayBold, fontSize: 30, color: C.gold },
