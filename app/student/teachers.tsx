...
// Browse Teachers: pulls the same `public_teachers` view the web app uses, with a
// search box and teacher cards (rating, experience, rate, specializations).

import { useCallback, useMemo, useState } from 'react';
import { StyleSheet, Text, View } from 'react-native';
import { useFocusEffect } from 'expo-router';
import { Ionicons } from '@expo/vector-icons';
import { Avatar, Card, Field, GradientHeader, Screen, EmptyState, Loading } from '@/components/ui';
...
export default function Teachers() {
  ...
  const filtered = useMemo(() => {
    ...
    return teachers.filter((t) => {
      const hay = [teacherName(t), t.country ?? '', ...(t.specializations ?? []), ...(t.teaching_languages ?? [])]
        .join(' ')
        .toLowerCase();
  ...
  if (loading) {
    return (
      <Screen scroll={false}>
        <Loading label="Finding teachers…" />
      </Screen>
    );
  }
  ...
  return (
    <Screen>
      <GradientHeader greeting="Browse" name="Find your teacher" subtitle={`${teachers.length} certified teachers available`} />
  ...
  return (
    <Card>
      <View style={styles.top}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.meta}>
            {(t.country ?? 'International')} · {t.years_experience ?? 0}yr exp
          </Text>
      ...
      <View style={styles.rateRow}>
        <View>
          <Text style={styles.rateLabel}>Per lesson</Text>
          <Text style={styles.rate}>${t.hourly_rate_usd ?? 0}</Text>
        </View>
        <View>
          <Text style={styles.rateLabel}>Trial</Text>
          <Text style={[styles.rate, { color: C.forest }]}>
            {Number(t.trial_rate_usd) > 0 ? `$${t.trial_rate_usd}` : 'Free'}
          </Text>
        </View>
      </View>
    </Card>
